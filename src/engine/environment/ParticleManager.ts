...
// Handles block destruction debris, footsteps, water splashes, torch embers, magic spores & weather drops
...
interface ActiveParticle {
  ...
  maxLife: number;
  gravity: number;
  type: 'debris' | 'footstep' | 'splash' | 'ember' | 'ash' | 'spore';
}
...
export class ParticleManager {
  ...
  public spawnBlockBreakParticles(pos: THREE.Vector3, blockType: BlockType): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const color = new THREE.Color(
        Math.max(0, Math.min(1, baseColor[0] + (Math.random() - 0.5) * 0.15)),
        Math.max(0, Math.min(1, baseColor[1] + (Math.random() - 0.5) * 0.15)),
        Math.max(0, Math.min(1, baseColor[2] + (Math.random() - 0.5) * 0.15))
      );
  ...
  public spawnWaterSplash(pos: THREE.Vector3): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      this.particles.push({
        ...
        velocity: new THREE.Vector3((Math.random() - 0.5) * 3.0, 3.0 + Math.random() * 2.5, (Math.random() - 0.5) * 3.0),
        color: new THREE.Color(0.4, 0.75, 0.98),
        size: 0.7,
        life: 0,
        maxLife: 0.5,
        gravity: 12.0,
  ...
  public update(deltaTime: number): void {
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      const scale = p.size * (1.0 - progress * 0.5);
